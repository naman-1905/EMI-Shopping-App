// Backend build stage: push_docker_image.groovy
// Pushes the backend Docker image to registries
...
def call(String imageName, String imageTag, String reg1, String reg2, String dockerCredId) {
    ...
    withCredentials([usernamePassword(credentialsId: dockerCredId, usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
        sh '''
            ...
            PUSH_IMAGE1=''' + reg1 + '''/emi_backend:''' + imageTag + '''
            PUSH_IMAGE1_LATEST=''' + reg1 + '''/emi_backend:latest
            
            # Tag for registry 2 (pull registry)
            PUSH_IMAGE2=''' + reg2 + '''/emi_backend:''' + imageTag + '''
            PUSH_IMAGE2_LATEST=''' + reg2 + '''/emi_backend:latest
            
            echo "Tagging and pushing to Registry 1 (Push): ''' + reg1 + '''"
            docker tag emi_backend:''' + imageTag + ''' $PUSH_IMAGE1
            docker tag emi_backend:latest $PUSH_IMAGE1_LATEST
            ...
            docker tag emi_backend:''' + imageTag + ''' $PUSH_IMAGE2
            docker tag emi_backend:latest $PUSH_IMAGE2_LATEST
            ...
            echo "✓ Backend Docker image pushed to both registries"
        '''
    }
}
